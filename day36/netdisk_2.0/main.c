        ...
        for(int i = 0; i < ready_fd_num; i++){
            if(evs[i].data.fd == sock_fd){
                ...
                if(control_code == 1){  // 注册
                    ...
                    if(ret){    // 数据库有用户名
                        send_protocol(evs[i].data.fd, REGISTER_FAILED, NULL, 0);
                    }else{
                        // 生成密文写入数据库
                        insert_new_user(conn, &user_passwd);
                        //注册成功
                        send_protocol(evs[i].data.fd,REGISTER_SUCCESS,NULL,0);
                    }
                    ...
                    if(login_flag){
                        // 向客户端发送成功标志
                        send_protocol(evs[i].data.fd, LOGIN_SUCCESS, NULL, 0);
                    }
                    else{
                        send_protocol(evs[i].data.fd,  LOGIN_FAILED, NULL, 0);
                    }
                }
                if(login_flag){
                    printf("主线程放任务到队列\n");
                    user_id = get_user_id(conn, user_passwd.username);
                    // 放入队列
