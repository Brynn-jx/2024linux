...
void client_handle(task_t *pnew, MYSQL *conn){
    ...
    while(1){
        ...
        case GETS:
            if(strlen(client_data)){
                do_gets(pnew, conn, client_data);
            }
            break;
        case PUTS:
            if(strlen(client_data))
                do_puts(pnew, conn, client_data);
            break;
        default:
            break;
        }
...
void do_puts(task_t *pnew, MYSQL *conn, char *file_name){
    ...
    int recvLen = 0;
    // 接收MD5的长度
    recvn(pnew->new_fd, &recvLen, 4);   
    recvn(pnew->new_fd, md5_str, recvLen);
    // 和数据库中的表file_md5sum查询看是否能秒传
    uint64_t second_transfer;//秒传标志
    second_transfer = check_file_md5_exist(pnew, conn, md5_str);
    if(second_transfer){    // 数据库中有对应的文件秒传成功
        // 插入file_system一条记录
        creat_file(pnew, conn, file_name, md5_str);
        send_protocol(pnew->new_fd, SUCCESS, NULL, 0);
        printf("秒传成功    \n");
    }
    else{
        printf("开始上传文件\n");
        send_protocol(pnew->new_fd, FAILED, NULL, 0);
        // 服务器使用splice使用零拷贝技术接收客户端传送过来的文件
        off_t file_size = recvFile(pnew->new_fd, md5_str);
        // 在file_md5sum新增一条记录
        creat_file_md5(conn,md5_str,file_size);
        // 插入file_system一条记录
        creat_file(pnew, conn, file_name, md5_str);
        send_protocol(pnew->new_fd, SUCCESS, NULL, 0);
        printf("上传成功\n");
...
void do_gets(task_t *pnew, MYSQL *conn, char *file_name_recv_size){
    ...
    seek_pos = get_file_name_pos(file_name_recv_size);
    char md5_str[50] = {0};
    get_md5_and_file_size(pnew, conn, md5_str, &file_size, file_name_recv_size);
