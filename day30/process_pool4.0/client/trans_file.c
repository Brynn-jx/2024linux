...
int recvFile(int new_fd)
{
    ...
    char* p = (char *)mmap(NULL, fileSize, O_WRONLY, MAP_SHARED, fd, 0); //这里必须先打开和fileSize一样大小的文件
    ERROR_CHECK(p, (char *)-1, "mmap");
    //再接文件内容 接受字节数为1000的整数倍
    while(fileSize - downLoadSize >= 1000)
    {
        recvn(new_fd,p + downLoadSize,1000);//接recvLen长度的数据
        // write(fd,buf,1000);//把接收到的内容写入文件
        downLoadSize+=1000;
    ...
    // 处理小于1000的时候
    recvn(new_fd,p + downLoadSize,fileSize - downLoadSize);//接recvLen长度的数据
    // write(fd,buf,fileSize - downLoadSize);//把接收到的内容写入文件
    printf("100.00%%\n");
    munmap(p, fileSize); // 解除映射写回磁盘
