...
int transFile(int new_fd){  // 发送文件
    ...
    char* p = (char *)mmap(NULL, file_buf.st_size, MAP_SHARED, O_RDONLY, fd, 0);    // 如果多个进程映射的是同一个文件且flag为MAP_SHARED则会共享同一块物理内存
    ERROR_CHECK(p, (char*)-1, "mmap");
    long ret = send(new_fd, p, file_buf.st_size, 0);   // 内部会循发送，所以可以直接发送大文件，而不用我们将大文件拆成小文件循环发送
    ERROR_CHECK(ret, -1, "send");
    munmap(p, file_buf.st_size);    // 解除映射
end:
    close(fd);
    return 0;
}
